...
function checkErgoIdentifier(input) {
  ...
  if (input.length == 64) {
    try {
      $.get(`${API_HOST}transactions/${input}`, function (data) {
        searchType = "1";
      });
    } catch {}
    ...
    try {
      $.get(
        `${API_HOST}mempool/transactions/byAddress/${input}`,
        function (data) {
    ...
    try {
      $.get(`${API_HOST}tokens/${input}`, function (data) {
        searchType = "2";
      });
    } catch {}

    try {
      $.get(`${API_HOST}blocks/${input}`, function (data) {
        searchType = "3";
      });
    } catch {}
  } else {
...
function formatErgValueString(value, maxDecimals = 4, force = false) {
  let ergValue = parseFloat(
    new BigNumber(value).dividedBy(1000000000).toString(),
  );
...
function formatDateString(dateString) {
  ...
  return (
    ...
    ", " +
    date.toLocaleDateString(getLang())
  );
}
...
function formatIpfsCidHtmlString(cid) {
  ...
  for (let i = 0; i < IPFS_PROVIDER_HOSTS.length; i++) {
    ...
    urlHtml +=
      `<p>Mirror 0${i + 1}: <a  target="_new" href="` +
      mirrorUrl +
      '">' +
      linkString +
      "</a></p>";
...
function formatValue(value, digits, autodigits = false, same = false) {
  ...
  if (same) {
    ...
    return (
      '<span title="' + vstring + '">' + nFormatter(value, digits) + "</span>"
    );
  }
}
...
function formatAssetDollarPriceString(tokenAmount, tokenDecimals, tokenId) {
  ...
  return formatDollarPriceString(
    formatAssetDollarPrice(tokenAmount, tokenDecimals, tokenId),
  );
}
...
function getAssetTitleParams(
  ...
  if (name == "Mew Fun Lottery Ticket") {
    imgSrc =
      "https://api.ergexplorer.com/nftcache/bafybeie6z4zm7ahjvlawjfq4idojdrahklksygpfmb4zvlrx3id3h5dyty.png";
  }
  ...
  let iconHtml =
    '<img style="display: none;" onload="onTokenIconLoad(this)"  class="token-icon" src="' +
    imgSrc +
    '"/>';
  ...
  return (
    '<a title="' +
    (scam ? "Reported as suspicious by users." : "") +
    '" class="' +
    (scam ? "text-danger" : "") +
    '" href="' +
    getTokenUrl(tokenId) +
    '">' +
    (iconIsToTheLeft ? iconHtml + " " : "") +
    (name == "" || name == null ? formatAddressString(tokenId, 15) : name) +
    (iconIsToTheLeft ? "" : " " + iconHtml) +
    "</a>"
  );
}
...
function formatBox(box, trueBox = false, unspent = false) {
  ...
  if (box.boxId) {
    customIdString =
      '<p><strong class="text-white">Box Id: </strong><a href=" ' +
      getBoxUrl(box.boxId) +
      '">' +
      box.boxId.substr(0, 8) +
      "..." +
      box.boxId.substr(box.boxId.length - 4) +
      '</a> <a title="' +
      box.boxId +
      '" onclick="copyId(event, this)" href="Copy to clipboard!">&#128203;</a></p>';
  }

  //Address
  addAddress(box.address);
  formattedData +=
    '<div class="ps-0 pe-0 pe-md-2 ps-md-2 col-9">' +
    customIdString +
    '<span><strong>Address: </strong></span><a class="address-string" addr="' +
    box.address +
    '" href="' +
    getWalletAddressUrl(box.address) +
    '" >' +
    formatAddressString(box.address, 8) +
    '</a> <a title="' +
    box.address +
    '" onclick="copyId(event, this)" href="Copy to clipboard!">&#128203;</a></p>';
  ...
  if (trueBox) {
    formattedData +=
      '<p><strong class="text-white">Transaction Id: </strong><a href=" ' +
      getTransactionsUrl(box.transactionId) +
      '">' +
      box.transactionId +
      '</a> <a title="' +
      box.transactionId +
  ...
  if (trueBox) {
    if (box.spentTransactionId) {
      formattedData +=
        '<p><strong class="text-white">Spent Transaction Id: </strong><a href=" ' +
    ...
    formattedData +=
      '<p><strong class="text-white">Creation height</strong>: ' +
      nFormatter(box.creationHeight, 0, true, true) +
      "</p>";
  }

  if (box.settlementHeight) {
    formattedData +=
      '<p><strong class="text-white">Settlement height</strong>: <a href="' +
      getBlockUrl(box.blockId) +
      '">' +
      nFormatter(box.settlementHeight, 0, true, true) +
      "</a></p>";
  }
  ...
  if (!unspent) {
    formattedData +=
      '<div class="ps-0 pe-0 pe-md-2 ps-md-2 col-3 d-flex justify-content-end">' +
      (box.spentTransactionId === undefined
        ? ""
        : box.spentTransactionId === null
          ? '<span class="text-success">Unspent'
          : '<span class="text-danger">Spent') +
      "</span></div>";
  }

  //Value
  formattedData +=
    '<div style="padding-bottom:10px;" class="ps-0 pe-0 pe-md-2 ps-md-2 col-10"><span><strong>Value: </strong></span><span class="">' +
    formatErgValueString(box.value, 9, true, !trueBox) +
    ' <span class="text-light">' +
    formatAssetDollarPriceString(box.value, ERG_DECIMALS, "ERG") +
    "</span></span></div>";

  //Output transaction
  if (box.outputTransactionId != undefined) {
    formattedData +=
      '<div class="ps-0 pe-0 pe-md-2 ps-md-2 col-2 d-flex justify-content-end"><a href="' +
      getTransactionsUrl(box.outputTransactionId) +
      '" >Output</a></div>';
  }
  ...
  if (box.assets != undefined && box.assets.length > 0) {
    formattedData +=
      '<h5 class="ps-0 pe-0 pe-md-2 ps-md-2"><strong>Tokens:</strong></h5><div style="max-height:300px;overflow-y:auto;" class="ps-0 pe-0 pe-md-2 ps-md-2">';
    for (let j = 0; j < box.assets.length; j++) {
      let asset = box.assets[j];
      let assetPrice = formatAssetDollarPrice(
        asset.amount,
        asset.decimals,
        asset.tokenId,
      );
      ...
      formattedData +=
        "<p><strong>" +
        getAssetTitle(asset, true) +
        '</strong>: <span class="text-white">' +
        formatAssetValueString(asset.amount, asset.decimals, 4, !trueBox) +
        " " +
        (assetPrice == -1
          ? ""
          : '<span class="text-light">' +
            formatDollarPriceString(assetPrice) +
            "</span>") +
        "</span></p>";
    }
    formattedData += "</div>";
    ...
    if (registerKeys.length > 0) {
      for (let i = 0; i < registerKeys.length; i++) {
        ...
        if (
          ...
          if (!shownRegisters) {
            formattedData +=
              '<div style="margin-top:5px;" class="ps-0 pe-0 pe-md-2 ps-md-2 col-10"><p style="margin-bottom:5px;"><strong class="text-white">Additional registers:</strong></p>';
            shownRegisters = true;
          }
        }
  ...
  if (trueBox) {
    formattedData +=
      '<p> </p><p style="margin-bottom:5px;"><strong class="text-white">Ergo tree:</strong></p> <div style="word-wrap:break-word;background: var(--striped-1);" class="div-cell-dark">' +
      box.ergoTree +
      "</div>";
  }
...
function getAddressesInfo() {
  var jqxhr = $.post(
    ERGEXPLORER_API_HOST + "addressbook/getAddressesInfo",
    { addresses: addresses },
    function (data) {
  ...
  for (var i = 0; i < addressbook.length; i++) {
    if (addressbook[i]["address"] == address) {
      ...
      if (addressbook[i]["urltype"] != "") {
        ...
        if (shortAdd == "88dh") {
          shortAdd = addressbook[i]["address"].substr(
            addressbook[i]["address"].length - 4,
          );
        }
...
function formatTxAddressString(
  address,
  ...
  let addressString =
    '<a title="' +
    address +
    '" class="address-string" addr="' +
    address +
    '" href="' +
    getWalletAddressUrl(address) +
    '" >' +
    (getOwner(address) == undefined ? formattedAddress : getOwner(address)) +
    "</a>";
  if (address == AddressType.NA) {
    addressString = '<span class="text-light">' + AddressType.NA + "</span>";
  } else if (address == AddressType.Multiple) {
    addressString =
      '<span class="text-light" title="This transaction has multiple receiving addresses. Check transaction link for more details.">' +
  ...
  addressString =
    '<a title="' +
    address +
    '" onclick="copyAddress(event, this)" href="Copy to clipboard!">&#128203;</a> ' +
    addressString;
