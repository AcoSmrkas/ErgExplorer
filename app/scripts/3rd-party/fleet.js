    ...
    1: [
      function (require, module, exports) {
        ...
        function assertTypeOf(obj, expected) {
          ...
          if (type !== expected) {
            throw new Error(
              `Expected an object of type '${expected}', got '${type}'.`,
            );
          }
        }
        ...
        function hasDuplicatesBy(array, selector) {
          return array.some((item, index) => {
            return (
              array.findIndex((x) => selector(x) === selector(item)) !== index
            );
          });
        }
        ...
        function depthOf(array) {
          return Array.isArray(array)
            ? 1 + Math.max(0, ...array.map(depthOf))
            : 0;
        }
        ...
        function utxoDiff(minuend, subtrahend) {
          ...
          for (const token of minuend.tokens) {
            const balance =
              token.amount -
              (subtrahend.tokens.find((t) => t.tokenId === token.tokenId)
                ?.amount || _0n);
            if (balance !== _0n) {
            ...
            if (isDefined(options) && isDefined(options.index)) {
              ...
              if (this._isIndexOutOfBounds(options.index)) {
                throw new RangeError(
                  `Index '${options.index}' is out of range.`,
                );
              }
  ...
  > ${tokenId}: ${amount?.toString()}`;
        ...
        var AccumulativeSelectionStrategy = class {
          ...
          select(inputs, target) {
            ...
            const selectedNanoErgs = common.sumBy(
              selection,
              (input) => input.value,
            );
            if (
              (common.isUndefined(target.nanoErgs) &&
                common.isEmpty(target.tokens)) ||
              (common.isDefined(target.nanoErgs) &&
                selectedNanoErgs < target.nanoErgs)
            ) {
              const targetAmount = common.isDefined(target.nanoErgs)
          ...
          _selectTokens(targets) {
            ...
            for (const target of targets) {
              const targetAmount = common.isDefined(target.amount)
                ? target.amount - common.utxoSum(selection, target.tokenId)
                : void 0;
              if (targetAmount && targetAmount <= common._0n) {
                continue;
              }
              selection = selection.concat(
                this._select(targetAmount, target.tokenId),
              );
            }
            return selection;
          }
          _select(target, tokenId) {
            ...
            if (common.some(selection)) {
              this._inputs = this._inputs.filter(
                (input) => !selection.includes(input),
              );
            }
            return selection;
          }
          ...
          select(target) {
            ...
            if (common.some(selected)) {
              ...
              if (remaining.nanoErgs && remaining.nanoErgs > common._0n) {
                remaining.nanoErgs -= common.sumBy(
                  selected,
                  (input) => input.value,
                );
              }
              if (
                common.some(remaining.tokens) &&
                selected.some((input) => !common.isEmpty(input.assets))
              ) {
            ...
            selected = selected.concat(
              this._strategy.select(unselected, remaining),
            );
            ...
            for (const box of boxes) {
              ...
              for (const token of box.assets) {
                tokens[token.tokenId] =
                  (tokens[token.tokenId] || common._0n) +
                  common.ensureBigInt(token.amount);
              }
            }
            return {
              nanoErgs,
        ...
        var P2SH_ERGOTREE_PREFIX = crypto.hex.decode(
        ...
        function _getErgoTreeType(ergoTree) {
          if (
            ...
            common.startsWith(ergoTree, P2PK_ERGOTREE_PREFIX)
          ) {
            return common.AddressType.P2PK;
          } else if (
            ergoTree.length === P2SH_ERGOTREE_LENGTH &&
            common.startsWith(ergoTree, P2SH_ERGOTREE_PREFIX) &&
            common.endsWith(ergoTree, P2SH_ERGOTREE_SUFFIX)
          ) {
        ...
        var ErgoAddress = class _ErgoAddress {
          ...
          static validate(address) {
            ...
            const checksum = bytes.subarray(
              bytes.length - CHECKSUM_LENGTH,
              bytes.length,
            );
          ...
          toP2SH(network) {
            ...
            const hash = crypto
              .blake2b256(this._ergoTree)
              .subarray(0, P2SH_HASH_LENGTH);
        ...
        var ErgoUnsignedTransaction = class {
          ...
          get burning() {
            return common.utxoDiff(
              common.utxoSum(this.inputs),
              common.utxoSum(this.outputs),
            );
          }
        ...
        var TokensCollection = class extends common.Collection {
          ...
          _map(token) {
            return {
              tokenId: token.tokenId,
              amount: common.ensureBigInt(token.amount),
            };
          }
          _addOne(token, options) {
            if (
              common.isUndefined(options) ||
              (options.sum && !common.isDefined(options.index))
            ) {
          ...
          remove(tokenIdOrIndex, amount) {
            ...
            if (typeof tokenIdOrIndex === "number") {
              if (this._isIndexOutOfBounds(tokenIdOrIndex)) {
                throw new RangeError(
                  `Index '${tokenIdOrIndex}' is out of range.`,
                );
              }
          ...
          remove(boxIdOrIndex) {
            ...
            if (typeof boxIdOrIndex === "number") {
              if (this._isIndexOutOfBounds(boxIdOrIndex)) {
                throw new RangeError(
                  `Index '${boxIdOrIndex}' is out of range.`,
                );
              }
              index = boxIdOrIndex;
            } else {
              index = this._items.findIndex(
                (box) => box.boxId === boxIdOrIndex,
              );
              if (this._isIndexOutOfBounds(index)) {
                throw new NotFoundError(
            ...
            if (basis) {
              ...
              if (common.some(basis.tokens)) {
                for (const token of basis.tokens) {
                  ...
                  tokens[token.tokenId] =
                    (tokens[token.tokenId] || common._0n) + token.amount;
                }
              }
            }
        ...
        var ErgoTree = class {
          ...
          get hasSegregatedConstants() {
            return (
              (this.header & common.ergoTreeHeaderFlags.constantSegregation) !=
              0
            );
          }
          get hasSize() {
            return (
              (this.header & common.ergoTreeHeaderFlags.sizeInclusion) != 0
            );
          }
          toBytes() {
            return this._bytes;
        ...
        var OutputBuilder = class {
          ...
          constructor(value, recipient, creationHeight) {
            ...
            if (typeof recipient === "string") {
              this._address = common.isHex(recipient)
          ...
          get value() {
            return common.isDefined(this._valueEstimator)
          ...
          setCreationHeight(height, options) {
            if (
              ...
              (options.replace === false &&
                common.isUndefined(this._creationHeight))
            ) {
              ...
              if (common.isEmpty(this.additionalRegisters)) {
                this.setAdditionalRegisters({
                  R4: serializer
                    .SColl(
                      serializer.SByte,
                      crypto.utf8.decode(this.minting.name || ""),
                    )
                    .toHex(),
                  R5: serializer
                    .SColl(
                      serializer.SByte,
                      crypto.utf8.decode(this.minting.description || ""),
                    )
                    .toHex(),
                  R6: serializer
                    .SColl(
                      serializer.SByte,
                      crypto.utf8.decode(
                        this.minting.decimals?.toString() || "0",
                      ),
                    )
                    .toHex(),
                });
        ...
        function createPluginContext(transactionBuilder) {
          return {
            ...
            addDataInputs: (dataInputs, options) =>
              transactionBuilder.withDataFrom(dataInputs, options).dataInputs
                .length,
            burnTokens: (tokens) => {
        ...
        var TransactionBuilder2 = class {
          ...
          sendChangeTo(address) {
            if (typeof address === "string") {
              this._changeAddress = common.isHex(address)
                ? ErgoAddress.fromErgoTree(address, common.Network.Mainnet)
          ...
          build() {
            ...
            const target = common.some(this._burning)
              ...
              if (change.nanoErgs > common._0n) {
                if (common.some(changeBoxes)) {
                  if (this.settings.shouldIsolateErgOnChange) {
                    ...
                    firstChangeBox.setValue(
                      firstChangeBox.value + change.nanoErgs,
                    );
                  }
        ...
        function estimateChangeSize({
          ...
          size += tokens.reduce(
            (acc, curr) =>
              (acc +=
                common.byteSizeOf(curr.tokenId) +
                serializer.estimateVLQSize(curr.amount)),
            0,
          );
          if (tokens.length > maxTokensPerBox) {
            if (tokens.length % maxTokensPerBox > 0) {
              size +=
                serializer.estimateVLQSize(maxTokensPerBox) *
                Math.floor(tokens.length / maxTokensPerBox);
              size += serializer.estimateVLQSize(
                tokens.length % maxTokensPerBox,
              );
            } else {
    ...
    3: [
      ...
      {
        "@fleet-sdk/common": 1,
        "@noble/hashes/blake2b": 9,
        "@noble/hashes/sha256": 11,
        "@noble/hashes/utils": 12,
        "@scure/base": 13,
      },
    ],
    4: [
      function (require, module, exports) {
        ...
        function isColl(type) {
          return (
            ...
            type.code <=
              descriptors.coll.nestedCollTypeCode + MAX_PRIMITIVE_TYPE_CODE
          );
        }
          ...
          coerce(elements) {
            if (
              ...
              return typeof elements === "string"
                ? crypto.hex.decode(elements)
                : Uint8Array.from(elements);
            }
        ...
        var SSigmaProp = monoProxy(SSigmaPropType, descriptors.sigmaProp);
              ...
              switch (type.code) {
                ...
                case descriptors.sigmaProp.code: {
                  ...
                  if (node.type === descriptors.groupElement) {
                    ...
                    return _DataSerializer.serialize(
                      node.data,
                      node.type,
                      writer,
                    );
            ...
            if (type.embeddable) {
              ...
              switch (type.code) {
                ...
                case descriptors.tuple.code: {
                  return type.elementsType.map((t) =>
            ...
            if (type.embeddable) {
              ...
              if (type.elementsType.embeddable) {
                ...
                if (nestedColl.elementsType.embeddable) {
                  writer.write(
                    descriptors.coll.nestedCollTypeCode +
                      nestedColl.elementsType.code,
                  );
                } else {
                ...
                case 2: {
                  ...
                  if (left.embeddable) {
                    if (left.code === right.code) {
                      writer.write(
                        descriptors.tuple.symmetricPairTypeCode + left.code,
                      );
                    } else {
                      writer.write(
                        descriptors.tuple.pairOneTypeCode + left.code,
                      );
                      this.serialize(right, writer);
                    }
                  } else if (right.embeddable) {
                    writer.write(
                      descriptors.tuple.pairTwoTypeCode + right.code,
                    );
              ...
              switch (ctorCode) {
                ...
                case constructorCode.simpleColl: {
                  const internal =
                    embdCode === 0
                      ? this.deserialize(r)
                      : getPrimitiveType(embdCode);
                  return new SCollType(internal);
                }
                case constructorCode.nestedColl: {
                  return new SCollType(
                    new SCollType(getPrimitiveType(embdCode)),
                  );
                }
                case constructorCode.pairOne: {
                  const internal =
                    embdCode === 0
                      ? [this.deserialize(r), this.deserialize(r)]
                      : [getPrimitiveType(embdCode), this.deserialize(r)];
        ...
        function isBox(box) {
          const castedBox = box;
          return (
            common.isDefined(castedBox.transactionId) &&
            common.isDefined(castedBox.index)
          );
        }
          ...
          if (common.some(tokenIds)) {
            tokens.map((token) =>
              writer
                ...
                .writeBigVLQ(common.ensureBigInt(token.amount)),
            );
          } else {
            tokens.map((token) =>
              writer
                .writeHex(token.tokenId)
                .writeBigVLQ(common.ensureBigInt(token.amount)),
            );
          }
        }
        ...
        function estimateBoxSize(box, withValue) {
          if (common.isUndefined(box.creationHeight)) {
            throw new Error(
              "Box size estimation error: creation height is undefined.",
            );
          }
          let size = 0;
          size += estimateVLQSize(
            common.isDefined(withValue) ? withValue : box.value,
          );
          ...
          size += box.assets.reduce(
            (acc, curr) =>
              (acc +=
                common.byteSizeOf(curr.tokenId) + estimateVLQSize(curr.amount)),
            0,
          );
        ...
        function serializeTransaction(transaction) {
          ...
          transaction.dataInputs.map((dataInput) =>
            writer.writeHex(dataInput.boxId),
          );
      ...
      { "@fleet-sdk/common": 1, "@fleet-sdk/crypto": 3 },
    ],
    5: [
      function (require, module, exports) {
        "use strict";
        ...
        class BLAKE2 extends utils_js_1.Hash {
          constructor(
            ...
            if (
              ...
              throw new Error(
                `key must be up 1..${keyLen} byte long or undefined`,
              );
          ...
          _cloneInto(to) {
            const { buffer, length, finished, destroyed, outputLen, pos } =
              this;
    ...
    0xade682d1, 0x510e527f, 0x2b3e6c1f, 0x9b05688c, 0xfb41bd6b, 0x1f83d9ab, 0x137e2179, 0x5be0cd19
        ...
        class BLAKE2b extends _blake2_js_1.BLAKE2 {
          constructor(opts = {}) {
            super(
              128,
              opts.dkLen === undefined ? 64 : opts.dkLen,
              opts,
              64,
              16,
              16,
            ...
            this.v0l ^=
              this.outputLen | (keyLength << 8) | (0x01 << 16) | (0x01 << 24);
            if (opts.salt) {
      ...
      { "./_blake2.js": 6, "./_u64.js": 8, "./utils.js": 12 },
    ],
    10: [
      function (require, module, exports) {
        "use strict";
      ...
      function (require, module, exports) {
        ...
        function chain(...args) {
          ...
          const encode = Array.from(args)
            .reverse()
            .reduce(
              (acc, i) => (acc ? wrap(acc, i.encode) : i.encode),
              undefined,
            );
          ...
          return {
            ...
            decode: (input) => {
              ...
              return input.map((letter) => {
                ...
                if (index === -1)
                  throw new Error(
                    `Unknown letter: "${letter}". Allowed: ${alphabet}`,
                  );
                return index;
              });
            },
        ...
        const gcd = /* @__NO_SIDE_EFFECTS__ */ (a, b) =>
          !b ? a : gcd(b, a % b);
          ...
          return {
            ...
            decode: (digits) => {
              ...
              return Uint8Array.from(
                convertRadix2(digits, bits, 8, revPadding),
              );
            },
          };
        }
        ...
        exports.base58xmr = {
          ...
          decode(str) {
            ...
            for (let i = 0; i < str.length; i += 11) {
              ...
              res = res.concat(
                Array.from(block.slice(block.length - blockLen)),
              );
            }
            return Uint8Array.from(res);
          },
        };
        const base58check = (sha256) =>
          chain(
    ...
    14: [
      ...
      { "@fleet-sdk/core": 2, "@fleet-sdk/serializer": 4 },
    ],
  },
  {},
  [14],
