...
function printAddresses() {
  var jqxhr = $.get(
    ERGEXPLORER_API_HOST +
      "addressbook/getAddresses?offset=" +
      offset +
      "&limit=" +
      ITEMS_PER_PAGE +
      "&type=" +
      ...
      if (data.total > 0) {
        for (let i = 0; i < items.length; i++) {
          ...
          if (lastName == item.name) {
            ...
            formattedResult +=
              '<div class="col-12 mb-md-3"><div class="row div-cell-dark">';
            formattedResult +=
            ...
            formattedResult +=
              '<p><strong>Type: </strong><span class="' +
              spanClass +
              '">' +
              type +
              "</span></p>";
            ...
            if (item.url) {
              formattedResult +=
                '<p><strong>Url: </strong><a target="_new" href="' +
                item.url +
                '">' +
                item.url +
                "</a></p>";
            ...
            if (data.tokens.length > 0) {
              for (let j = 0; j < data.tokens.length; j++) {
                ...
                if (token.addressname == item.name) {
                  formattedResult +=
                    '<br><p><strong>Token:</strong></p><a style="word-wrap:break-word;" href="' +
                    getTokenUrl(token.id) +
                    '">' +
                    token.id +
                    "</a>";
                  break;
        ...
        formattedResult +=
          '<div class="div-cell-dark">There are no entries in the address book.</div>';
        $("#pagination").remove();
      }
