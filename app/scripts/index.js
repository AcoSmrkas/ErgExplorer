...
function getNetworkState() {
  ...
  if (networkType == "testnet") {
    networkStateUrl =
      "https://api-testnet.ergoplatform.com/api/v1/networkState";
  }

  var jqxhr = $.get(networkStateUrl, function (data) {
    $("#blockHeight").html(
      data.height.toLocaleString("en-US", {
        maximumFractionDigits: 0,
        minimumFractionDigits: 0,
      }),
    );
    $("#totalTransactions").html(
      data.maxTxGix.toLocaleString("en-US", {
        maximumFractionDigits: 0,
        minimumFractionDigits: 0,
      }),
    );
...
function getPoolStats() {
  $.get("https://api2.mewfinance.com/dex/getTop10Volume", function (data) {
    ...
    for (let i = 0; i < poolStatsData.length; i++) {
      ...
      formattedResult +=
        '<td><a href="' +
        getTokenUrl(poolStat.asset) +
        '">' +
        getAssetTitleParams(null, poolStat.asset, poolStat.name, true) +
        "</a></td>";
...
function printGainersLosers(timeframe) {
  ...
  for (var i = data.items.length - 1; i >= 0; i--) {
    ...
    if (item.originaltimestamp != lastTimestamp || item.ticker == "ERG") {
  ...
  for (let i = 0; i < end; i++) {
    ...
    formattedResult +=
      '<td><a href="' +
      getTokenUrl(item.tokenid) +
      '">' +
      getAssetTitleParams(item, item.tokenid, item.ticker, true) +
      "</a></td>";
    ...
    formattedResult +=
      "<td>$" + formatValue(prices[item.tokenid], decimals) + "</td>";

    //Change
    formattedResult +=
      '<td><span class="' + classString + '">' + difference + "%</span></td>";
    ...
    .then((arrayBuffer) => {
      ...
      for (var i = 0; i < items.length; i++) {
        ...
        formattedResult +=
          ...
          formatDateString(parseInt(item.time)) +
          "</span></td>";
        ...
        formattedResult +=
          ...
          (fromAddress == "N/A"
            ...
            : '<a class="address-string" addr="' +
              fromAddress +
              '" href="' +
              getWalletAddressUrl(fromAddress) +
              '" >' +
              (getOwner(fromAddress) == undefined
                ? formatAddressString(fromAddress, 10)
                : getOwner(fromAddress)) +
              "</a>") +
          "</td>";
        ...
        formattedResult +=
          ...
          (toAddress == "N/A"
            ...
            : '<a class="address-string" addr="' +
              toAddress +
              '" href="' +
              getWalletAddressUrl(toAddress) +
              '" >' +
              (getOwner(toAddress) == undefined
                ? formatAddressString(toAddress, 10)
                : getOwner(toAddress)) +
              "</a>") +
          "</td>";
        ...
        //Value
        formattedResult +=
          '<td><span class="d-lg-none"><strong>Value: </strong></span>' +
          formatAssetValueString(item.value, item.decimals) +
          " " +
          getAssetTitleParams(item, item.tokenid, item.ticker, false) +
          ' <span class="text-light">' +
          formatDollarPriceString(
            (item.value / Math.pow(10, item.decimals)) * prices[item.tokenid],
          ) +
          "</span></td></tr>";
  ...
  xhr.addEventListener("readystatechange", function () {
    if (this.readyState === this.DONE) {
      ...
      $("#marketCap").html(
        "$" +
          (
            prices["ERG"] * getAssetValue(data.supply, ERG_DECIMALS)
...
function getStats() {
  ...
  var jqxhr = $.get(statsUrl, function (data) {
    ...
    //Average mining time
    $("#statsAverageMiningTime").html(
      "<p>" +
        millisToMinutesAndSeconds(data.blockSummary.averageMiningTime) +
        "</p>",
    );

    //Coins mined
    $("#statsCoinsMined").html(
      '<p class="text-white">' +
        formatErgValueString(data.blockSummary.totalCoins, 0) +
        ' <span class="text-light">' +
        formatAssetDollarPriceString(
          data.blockSummary.totalCoins,
          ERG_DECIMALS,
          "ERG",
        ) +
        "</span></p>",
    ...
    $("#statsNumberOfTransactions").html(
      "<p>" + data.transactionsSummary.total + "</p>",
    );

    //Total transaction fees
    $("#statsTotalTransactionFees").html(
      '<p class="text-white">' +
        formatErgValueString(data.transactionsSummary.totalFee) +
        ' <span class="text-light">' +
        formatAssetDollarPriceString(
          data.transactionsSummary.totalFee,
          ERG_DECIMALS,
          "ERG",
        ) +
        "</span></p>",
    ...
    //Mining cost
    //Blocks mined
    $("#statsTotalMinersRevenue").html(
      '<p class="text-white">' +
        formatErgValueString(data.miningCost.totalMinersRevenue) +
        ' <span class="text-light">' +
        formatAssetDollarPriceString(
          data.miningCost.totalMinersRevenue,
          ERG_DECIMALS,
          "ERG",
        ) +
        "</span></p>",
    ...
    //Average mining time
    $("#statsEarnedFromTransactionFees").html(
      "<p>" + data.miningCost.percentEarnedTransactionsFees + "%</p>",
    );

    //Coins mined
    $("#statsOfTransactionVolume").html(
      "<p>" + data.miningCost.percentTransactionVolume + "%</p>",
    );

    //Number of transactions
    $("#statsCostPerTransaction").html(
      '<p class="text-white">' +
        formatErgValueString(data.miningCost.costPerTransaction) +
        ' <span class="text-light">' +
        formatAssetDollarPriceString(
          data.miningCost.costPerTransaction,
          ERG_DECIMALS,
          "ERG",
        ) +
        "</span></p>",
    ...
    //Total output volume
    $("#statsHashRate").html(
      "<p>" + formatHashRateString(data.miningCost.hashRate) + "</p>",
    );
