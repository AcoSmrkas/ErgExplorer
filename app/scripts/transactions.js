...
function initSocket() {
  ...
  newSocket?.on("mempoolTxs", async (transactions) => {
...
function collectTokenIds(transactions) {
...
function getTxUrl(mempool) {
  ...
  if (mempool) {
    ...
    if (networkType == "testnet") {
      txUrl =
        "https://api-testnet.ergoplatform.com/transactions/unconfirmed/" + txId;
    }
  }
...
function getTransaction(mempool, retries = 0) {
  ...
  fetch(txUrl)
    .then(async (response) => {
      ...
      if (mempool) {
        ...
        if (walletAddress != "N/A") {
          let addressMempoolUrl =
            API_HOST_2 + "mempool/transactions/byAddress/" + walletAddress;

          if (networkType == "testnet") {
            addressMempoolUrl =
              API_HOST_2 +
              "api/v1/mempool/transactions/byAddress/" +
              walletAddress;
          }
    ...
    .catch(function () {
      if (mempool) {
        showLoadError(
          "No results matching your query.<br>Just submitted your transaction? Hang tight for a little longer!",
        );
        $("#txLoading").hide();
        ...
        if (retries < 5) {
          ...
          showLoadError(
            "No results matching your query.<br>Just submitted your transaction? Try reloading this page.",
          );
        }
      } else {
        getTransaction(true);
...
function printTransaction(data, mempool) {
  ...
  //Id
  $("#txHeader").html(
    '<p><a href="Copy to clipboard!" onclick="copyTransactionAddress(event)">' +
      data.id +
      " &#128203;</a></p>",
  );
  $("#txHeaderMobile").html(
    '<p><a href="Copy to clipboard!" onclick="copyTransactionAddress(event)">' +
      data.id.substr(0, 8) +
      "..." +
      data.id.substr(data.id.length - 4) +
      " &#128203;</a></p>",
  );
  ...
  if (mempool) {
    if (data.creationTimestamp) {
      $("#txTime").html(
        "<p>" + formatDateString(data.creationTimestamp) + "</p>",
      );
    }
  } else {
  ...
  if (mempool) {
    ...
    $("#txIncludedInBlocks").html(
      '<a href="' +
        getBlockUrl(data.outputs[0].blockId) +
        '">' +
  ...
  $("#txTotalCoinsTransferred").html(
    formatErgValueString(totalCoinsTransferred, 6) +
      " " +
      formatAssetDollarPriceString(totalCoinsTransferred, ERG_DECIMALS, "ERG"),
  );
  ...
  $("#txFees").html(
    formatErgValueString(fee, 5) +
      " " +
      formatAssetDollarPriceString(fee, ERG_DECIMALS, "ERG"),
  );
