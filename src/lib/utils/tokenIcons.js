...
import { hasTokenIcon, getTokenIcon, updateTokenIcons } from '$lib/stores/tokenIconsStore.js';
...
export async function getTokenIcons() {
	try {
		const response = await fetch(`${API_ENDPOINTS.ERGEXPLORER}tokens/getTokenIcons`);
...
export function getAssetTitleParams(token, tokenId, name, iconIsToTheLeft, scam = false) {
	...
	const displayName = (name == '' || name == null) ? 
		tokenId.substring(0, 15) + (tokenId.length > 15 ? '...' : '') : name;
...
export function formatAddressLink(address, displayLength = 10) {
	...
	const displayAddress = address.substring(0, displayLength) + (address.length > displayLength ? '...' : '');
...
export function getAddressLinkProps(address, name = '', startChars = 9, endChars = 3, showCopy = true) {
	return {
		address,
		name,
		startChars,
