...
export function initializeGlobalAddressHover() {
			...
			if (link) {
				...
				if (!relatedTarget || 
					(!link.contains(relatedTarget) && 
					 (!relatedTarget.closest || !relatedTarget.closest('.address-popup')))) {
					hideAddressPopup();
				}
			}
		}
	...
	document.addEventListener('mouseover', (event) => {
		if (event.target && typeof event.target.closest === 'function' && event.target.closest('.address-popup')) {
			...
			if (popup) {
				...
				if (!relatedTarget || 
					(!popup.contains(relatedTarget) && 
					 (!relatedTarget.closest || !relatedTarget.closest('[data-address]')))) {
					hideAddressPopup();
				}
			}
		}
