...
export function initializeGlobalTransactionHover() {
			...
			if (link) {
				...
				if (!relatedTarget || 
					(!link.contains(relatedTarget) && 
					 (!relatedTarget.closest || !relatedTarget.closest('.transaction-popup')))) {
	...
	document.addEventListener('mouseover', (event) => {
		if (event.target && typeof event.target.closest === 'function' && event.target.closest('.transaction-popup')) {
